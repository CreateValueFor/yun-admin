...
export default {
  name: 'exclude-menu-and-ingredient',
  props: {
    order: {
      type: Object,
    },
  ...
  async mounted() {
    ...
    this.excludeIngredient = this.order.Ingredients.map(
      (item) => this.ingredients.filter((igd) => igd.name === item.name)[0].id
    )
    this.excludeProduct = this.order.Products.map(
      (item) => this.products.filter((prd) => prd.name === item.name)[0].id
    )
